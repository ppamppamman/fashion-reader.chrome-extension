  ...
  switch(request.method){
    ...
    case "POST_SCENARIO_ITEM_INFO":
      ...
      chrome.storage.local.get(["cart"], async (result) => {
        ...
        await sendResponse({data: "POST 시나리오 넘어갔다 이말이야", cart:result.cart});
      });
      return true;
    case "GET_CART":
    ...
    case "PATCH_CURRENT_ID_LEFT":
      chrome.storage.local.get(["cart"], async (result) => {
        if(result.cart.currentId <= 0){
          await sendResponse({data: "currentID가 맨앞이다 이말이야", currentId: result.cart.currentId, itemCount:result.cart.items.length });
        } else {
          await chrome.storage.local.set({
            cart: { currentId: result.cart.currentId-=1, items: result.cart.items }
          });
          await sendResponse({data: "currentID를 -1 한다 이말이야", currentId: result.cart.currentId, itemCount:result.cart.items.length });
        }
      });
      return true;
    case "PATCH_CURRENT_ID_RIGHT":
      chrome.storage.local.get(["cart"], async (result) => {
        if(result.cart.currentId >= result.cart.items.length-1){
          await sendResponse({data: "currentID가 맨뒤다 이말이야", currentId: result.cart.currentId, itemCount:result.cart.items.length });
        } else {
          await chrome.storage.local.set({
            cart: { currentId: result.cart.currentId+=1, items: result.cart.items }
          });
